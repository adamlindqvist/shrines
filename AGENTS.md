...
Bridges are Z-aligned and permanently open once activated. Closed bridges rise from 2.4 units below their authored position over 0.8 gameplay seconds; their whole passage is blocked until fully open. Dynamic collision blockers are shared by the player, carried boxes and enemies. River shore openings derive from bridge footprints. Pause freezes rules, chests and bridges; reset restores locks, bridge positions/collision, zone visits and fired rules. `bridge-example` (Solbron) demonstrates box → plate → bridge → chest outside the main journey.
...
- Grabbing preserves the initial player–block distance. The pair translates together while the held box sweeps toward player facing at 1200 degrees per second, including while stationary. Rotation follows the shortest arc (clockwise for exact reversals), stops at obstacles, other blocks, bounds, or unwalkable height changes, and catches up when clear. Only when the block limits translation does remaining tangential input swivel the player around it; a player-only collision does not activate this fallback. Movement uses collision substeps of at most 0.05 world units. The held block uses circular clearance against the player so square corners cannot catch an orbit; other blocks retain their existing collision rules. Damage still applies while grabbing, but knockback is suppressed to preserve their spacing. The visual child eases up by 0.28 units while held, with a subtle bob and sandy grains on lift and landing. This presentation freezes on pause and settles after end states. Pause retains the grip; switch activation, end states, and reset release it.
